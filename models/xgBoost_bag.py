...
model_xg = xgb.XGBClassifier(learning_rate=0.1,gamma=0,max_depth=20,min_child_weight=1,max_delta_step=0,subsample=1,colsample_bytree=1,silent=1,seed=0,reg_lambda=1,reg_alpha=0, n_estimators=1000)

y = train['isDuplicate']
x = train.drop('isDuplicate', axis=1)

# Bagging Classifier
model = BaggingClassifier(base_estimator=model_xg, n_estimators=60, random_state=1)
...
joblib.dump(gbm_pred, 'gbm_pred_model_bag.pkl', compress=True)

id = test['id']
test.drop('id', axis=1, inplace=True)

print('Loading xgBoost model...')
gbm_pred = joblib.load('gbm_pred_model_bag.pkl')
...
print(result.head())
result.to_csv('Submission_1_bag.csv', encoding='utf-8', index=False)
